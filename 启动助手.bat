...
chcp 65001 >nul 2>&1
rem ============================================================================
rem  research-desk · 一键启动
rem
rem  双击本文件即可：后台启动 Streamlit 界面（完全无窗口）→ 等它就绪 →
rem  自动打开浏览器。
rem
rem  · 界面进程在后台独立运行，关掉浏览器标签页不会停掉它；
rem  · 要停止请双击「停止助手.bat」；
...
title research-desk · 启动中

rem ---------------------------------------------------------------- 可调参数
rem  默认用哪个数据库。也可在这里改成别的库文件。
set "DB=data\desk.db"
set "PORT=8501"
set "HOST=127.0.0.1"
...
if not exist "%SERVE%" (
  echo   [×] 找不到启动入口：%SERVE%
  echo       请确认本脚本与 scripts\serve_desk.py 在同一个项目目录下。
  echo.
  pause
  exit /b 1
)
...
rem ------------------------------------- 2. 界面选库：把路径交给环境变量
rem  界面读 RA_DESK_DB 决定用哪个库。这里只在库文件存在时设置，
rem  否则让引擎用它自己的默认库（并会在日志里说明）。
if exist "%ROOT%%DB%" (
  set "RA_DESK_DB=%ROOT%%DB%"
  echo   使用数据库：%DB%
) else (
  echo   未找到 %DB%，将使用引擎默认库。
)

rem ------------------------------------------- 3. 挑一个没被占用的端口
set /a TRIES=0
:find_port
call :is_our_app %PORT%
...
rem  后台完全不可见，也不会闪一下黑框。
set "VBS=%TEMP%\rd-launch-%RANDOM%%RANDOM%.vbs"
> "%VBS%" echo Set sh = CreateObject("WScript.Shell")
>>"%VBS%" echo sh.Run """%PYW%"" ""%SERVE%"" --port %PORT% --host %HOST%", 0, False
cscript //nologo "%VBS%" >nul 2>&1
del "%VBS%" >nul 2>&1

echo   界面已在后台启动（端口 %PORT%），正在等它就绪…
...
rem ------------------------------------------------------------------ 子过程
rem  :is_our_app ^<port^>  —— 返回 0 表示"本项目已在运行"
:is_our_app
