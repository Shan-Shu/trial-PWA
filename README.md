...
不需要碰命令行，直接**双击项目根目录的 `启动助手.bat`**：

1. 后台启动服务（用 `pythonw.exe` + 隐藏窗口，**完全看不到黑框**）；
2. 轮询 `/api/health` 直到真的能响应（而不是盲等固定秒数）；
3. 自动打开浏览器到写作台。
...
| 情况 | 行为 |
|---|---|
| 服务已在运行 | 不再起第二个进程，直接打开浏览器 |
| 8000 被别的程序占用 | 自动改用 8001、8002…（最多找 10 个） |
| 启动失败 | 窗口**保留**并打印日志路径 `data/logs/serve.log` |
| 想停止 | 双击 `停止助手.bat`（会跨端口找到它） |

想固定用哪个数据库，改 `启动助手.bat` 顶部的 `set "DB=..."` 一行；
也可以启动后在界面右上角的下拉框里切换。

> 两个 `.bat` 都是 UTF-8 编码，第 2 行的 `chcp 65001` 与编码配套，
> 且在此之前不出现任何中文——这是让中文在任意代码页下都不乱码的关键。
> 改动时请保持这个顺序，否则 cmd 会按系统默认代码页解析文件。
