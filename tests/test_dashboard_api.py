...
class DashboardApiTest(unittest.TestCase):
    ...
    def test_index_served(self):
        """根路由：旧 JS 前端已退役，现在返回一份"去哪找界面"的 JSON。

        注意 `assertIn` 是对**响应体**做的，而 body 里含 `research-desk`
        与 `research-agent` 两个词，所以这里断言服务标识，不是断言前端还在。
        """
        r = self.client.get("/")
        self.assertEqual(r.status_code, 200)
        payload = r.json()
        self.assertTrue(payload.get("ok"))
        self.assertEqual(payload.get("service"), "research-desk")
        self.assertIn("Streamlit", payload.get("ui", ""))
