...
def _consume_entry(**kwargs: Any) -> dict[str, Any]:
    ...
    # 必须先认调用方给的连接：派工可能是对**另一个库**下的单，
    # 落回默认库就会"下在 A 库、写进 B 库"（与其余登记项保持一致）。
    conn = kwargs.get("conn") or connect(
        kwargs.get("db_path") or default_settings.db_path)
    own_conn = kwargs.get("conn") is None
    try:
...
def _rebuild_ontology_entry(**kwargs: Any) -> dict[str, Any]:
    from research_agent.db import connect
    from research_agent.config import settings as default_settings
    from research_agent.ontology.store import init_ontology, rebuild_ontology_views

    conn = kwargs.get("conn") or connect(
        kwargs.get("db_path") or default_settings.db_path)
    own_conn = kwargs.get("conn") is None
    try:
        # 本体表是**懒建**的（`db.SCHEMA` 里没有），新建的库直接调
        # `rebuild_ontology_views` 会 `no such table: ontology_domain_members`。
        # 派工要能在任意库上独立跑通，所以这里自己保证 schema 存在（幂等）。
        init_ontology(conn)
