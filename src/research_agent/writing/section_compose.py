...
def _skeleton(heading: str, instruction: str, reason: str,
    ...
    if instruction:
        lines.extend(["", f"节点指令：{instruction}"])
...
def compose_section(
    *,
    ...
    """为单个大纲节点生成一段正文（或显式骨架草稿）。

    返回 ``{content, generated_by, citations, bindings, invalid_indices,
    material_count, model_error}``；**不落库**——落库由 ``section_service`` 统一做，
    保证"生成"与"状态流转"只有一个写入点。
    """
    ...
    if model is None:
        ...
        content = _skeleton(heading, instruction, model_error, materials)
    else:
        try:
